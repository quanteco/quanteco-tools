...
#' @export
#' @return a list of (1) a matrix of eigenvectors, (2) eigenvalues (scaled to represent the respective Moran's I value for each eigenvector), and (3) the neighborhood matrix used (sometimes different than one supplied if asymmsetric).
#' @details This is a function to perform principal coordinates of neighbor matrices (PCNM) on a weighted neighborhood matrix (\strong{W}). By performing PCNM on this matrix, we are able to use the eigenvectors as regressors within a linear model context, thus filtering any dependent structures in our data by partitioning the autocorrelation into synthetic variates.
#' 
